...
export default function ClaimPage() {
    ...
    fetch(`/api/properties/${objectId}`)
      ...
      .then((d) => {
        ...
        if (prop && prop.id) {
          ...
          setError('Property not found on the DUAL network.');
          setPhase('error');
        }
      })
      .catch(() => {
        setError('Could not verify this property.');
        setPhase('error');
      });
  }, [objectId]);
  ...
  const propertyName = property?.propertyData?.address || property?.propertyData?.name || 'DUAL Property';
  const propertyCity = property?.propertyData?.city || 'DUAL Network';
      ...
      <div className="w-full max-w-md">
        {phase === 'verifying' && (
          <div className="bg-white rounded-3xl shadow-lg p-8 text-center">
            ...
            <h2 className="text-lg font-bold text-slate-900">Verifying Property</h2>
        ...
        {phase === 'info' && property && (
          <div className="bg-white rounded-3xl shadow-lg overflow-hidden">
            <div className="wine-gradient p-6 text-center">
              ...
              <h2 className="text-lg font-bold text-white mt-2">{propertyName}</h2>
              <p className="text-wine-200 text-sm mt-1">{propertyCity}</p>
            </div>

            <div className="p-6 space-y-4">
              {(property.status === 'anchored' || property.provenance?.verified) && (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 rounded-xl p-3">
                  ...
                  <p className="text-sm font-semibold text-slate-900 capitalize">{property.propertyData?.propertyType || 'Property'}</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-3">
                  <p className="text-xs text-slate-500">On-Chain</p>
                  <p className="text-sm font-semibold text-slate-900 capitalize">{property.status || 'pending'}</p>
                </div>
              </div>
              ...
              <p className="text-xs text-slate-400 text-center">
                This property token will be added to your session wallet
              </p>
            </div>
          </div>
        )}
        ...
        {phase === 'claiming' && (
          <div className="bg-white rounded-3xl shadow-lg p-8 text-center">
            ...
            <h2 className="text-lg font-bold text-slate-900">Claiming Property</h2>
        ...
        {phase === 'success' && (
          <div className="bg-white rounded-3xl shadow-lg p-8 text-center">
            ...
            <h2 className="text-xl font-bold text-slate-900">Property Claimed!</h2>
            <p className="text-sm text-slate-500 mt-2">
              {propertyName} is now in your wallet.
            </p>
            <Link
              href="/wallet"
        ...
        {phase === 'already_claimed' && (
          <div className="bg-white rounded-3xl shadow-lg p-8 text-center">
            ...
            <p className="text-sm text-slate-500 mt-2">
              This property has already been claimed to your session wallet.
            </p>
            <Link
              href="/wallet"
